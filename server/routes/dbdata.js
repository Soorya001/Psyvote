...
const password = ""; // set it to your password of Oracle

const GetData = async (req, res) => {
  let connection;
  try {
    connection = await oracledb.getConnection({
      user: "", // set it to your username of Oracle
      password: password,
    ...
    const result = await connection.execute(`select sysdate from dual`); // write your query to be executed here
    res.send(result);
  } catch (err) {
    console.error(err);
  } finally {
